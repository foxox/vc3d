...
typedef _(dynamic_owns) struct Mesh
{
	...
	//GHOST MEMBERS
	_(ghost \bool edgeset[HEEdge*];)
	_(ghost \bool vertset[HEVert*];)
	_(ghost \bool faceset[HEFace*];)
	...
	_(invariant \forall size_t i; i < numverts <==> vertset[&verts[i]])
	_(invariant \forall size_t i; i < numedges <==> edgeset[&edges[i]])
	_(invariant \forall size_t i; i < numfaces <==> faceset[&faces[i]])
