...
_(pure) Vec3 Vec3GenVec3(float x, float y, float z)
{
	//#ifdef __cplusplus
		//Vec3 returnme(x,y,z);
	//#else
...
void Vec3NormalizeOut(Vec3* vec)
{
...
float Vec3LengthSquared(Vec3 vec)
{
...
Vec4 Vec4GenVec4(float r, float g, float b, float a)
{
	Vec4 returnme;
	returnme.r = r;
	returnme.g = g;
...
Mat4 Mat4GenTranslate(float x, float y, float z)
{
...
Mat4 Mat4Mat4Multiply(Mat4 a, Mat4 b)
{
	Mat4 returnme;
	uint i, j, k;
	for (i = 0; i < 4; i++)
...
Vec3 Vec3Mat4Transform(Vec3 vec, Mat4 mat)
{
	Vec3 returnme;
...
Vec3 Vec3Mat4TransformNormal(Vec3 vec, Mat4 mat)
{
	Vec3 returnme;
...
Mat4 Mat4GenLookAtTransform(Vec3 pos, Vec3 target, Vec3 up)
{
	...
	returnme = Mat4GenIdentity();
	z = Vec3Sub(pos, target);
		Vec3NormalizeOut(&z);
	x = Vec3CrossProduct(up, z);
		Vec3NormalizeOut(&x);
