...
typedef _(dynamic_owns) struct PairedLists
{
	...
	_(invariant \forall \natural i; {pairarray1+i} i < num1 ==> \mine(&pairarray1[i]))
	_(invariant \forall \natural j; {pairarray2+j} j < num2 ==> \mine(&pairarray2[j]))
	_(invariant \forall \natural i; /*{pairarray2+PairedArray1_witnesses[i]}*/ i < num1 ==> \mine(&pairarray2[PairedArray1_witnesses[i]]))
	_(invariant \forall \natural j; /*{pairarray1+PairedArray2_witnesses[j]}*/ j < num2 ==> \mine(&pairarray1[PairedArray2_witnesses[j]]))
...
//#endif
	...
	//free((void*) speccast(Paired[dis->num1], dis->pairarray1));
	//free((void*) speccast(Paired[dis->num2], dis->pairarray2));