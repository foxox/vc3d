...
procedure $stack_free(sf:int, x:$ptr);
  modifies $s;
  // TOKEN: the primitive extent of the object being reclaimed is mutable
  requires if $is_primitive($typ(x)) then $mutable($s, x);
  // TOKEN: the object extent of the object being reclaimed is mutable
  requires if !$is_primitive($typ(x)) then $extent_mutable($s, x);
